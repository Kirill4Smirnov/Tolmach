...
Укажите в `deploy/compose.env` Telegram ID администраторов:

```dotenv
TELEGRAM_ADMIN_USER_IDS=123456789
```

Администратор должен хотя бы один раз открыть личный чат с ботом и нажать
`Start`, иначе Telegram не разрешит боту доставлять ему заявки. Несколько ID
можно перечислить через запятую.

При первом запуске существующий `TELEGRAM_ALLOWED_USER_IDS`, если он задан,
однократно импортируется в SQLite. После этого доступ управляется кнопками в
заявках или командами `/users`, `/requests`, `/allow ID` и `/deny ID`; редактировать
конфигурацию для новых пользователей не требуется. Администраторские ID
остаются в конфигурации и не могут быть удалены через Telegram.
